...
class xDeepFM(object):
    def __init__(self):
        ...
        if Config.multi_features:
            for idx, s in enumerate(Config.multi_features):
                ...
                #print(self.valid_dict[self.ph['multi_index_%s' % s]],self.valid_dict[self.ph['multi_value_%s' % s]])
    ...
    def _train(self):
        ...
        with tf.Session() as self.sess:
            ...
            for i in range(Config.epochs):
                ...
                '''
                test_out= self.sess.run(self.softmax_output,feed_dict=self.test_dict)#placeholder可以使用 feed_dict 馈送数据
                test_output=test_out.tolist()
                with open('OUT/test_out_%s.csv'%(global_step/970),'w') as f:
    ...
    def _save_loss(self):
        loss_result = pd.DataFrame({
            ...
            'valid_loss':self.global_valid_loss,
