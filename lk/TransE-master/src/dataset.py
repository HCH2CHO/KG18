...
class KnowledgeGraph:
    ...
    def load_dicts(self):
        ...
        self.logging.info('-----Loading entity dict-----')
        ...
        self.logging.info('#entity: {}'.format(self.n_entity))
        self.logging.info('-----Loading relation dict-----')
        ...
        self.logging.info('#relation: {}'.format(self.n_relation))

    def load_triples(self):
        training_file = 'train.txt'
        validation_file = 'valid.txt'
        test_file = 'test.txt'
        self.logging.info('-----Loading training triples-----')
        ...
        self.logging.info('#training triple: {}'.format(self.n_training_triple))
        self.logging.info('-----Loading validation triples-----')
        ...
        self.logging.info('#validation triple: {}'.format(self.n_validation_triple))
        self.logging.info('-----Loading test triples------')
        ...
        self.logging.info('#test triple: {}\n\n'.format(self.n_test_triple))
    ...
    def get_training_batch(self,batch_size):
        ...
        while start < self.n_training_triple:
            ...
            pos=[self.training_triples[i] for i in rand_idx[start:end]]
            corrupt_head_prob = np.random.binomial(1, 0.5)
            neg=[]
            for head, tail, relation in pos:
                head_neg=head
                tail_neg=tail
                while True:
                    ...
                    if (head_neg, tail_neg, relation) not in self.training_triple_pool:
                        break
                neg.append((head_neg, tail_neg, relation))
            yield((pos,neg))
            start = end
    ...
    def generate_training_batch(self, in_queue, out_queue):
        while True:
            ...
            if raw_batch is None:
                self.logging.info("return directly!")
                return
            else:
                self.logging.info("NOT return directly!!!!")
                batch_pos = raw_batch
                batch_neg = []
                corrupt_head_prob = np.random.binomial(1, 0.5)
